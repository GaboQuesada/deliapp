    ...
    $.ajax({
        ...
        success: function (respuesta) {
            ...
            $.each(datosrespuesta, function (i, item) {
                ...
                $("#actividadesBox").append('<div class=" boxcar card " ><div class="card-body"><h5 class="card-title"><strong>' + datosrespuesta[i].nombre + '</strong></h5><p class=" boxcarcontent card-text">' + datosrespuesta[i].descripcion + '.</p> <button  onclick="putInModalActividades(' + datosrespuesta[i].id + ')" class="btn btn-warning  btn-sm" data-toggle="modal" data-target="#upDateCat"><i class="fas fa-pen-square"></i> Modificar</button>&nbsp;<button onclick="deleteActividades(' + datosrespuesta[i].id + ')" class="btn btn-danger btn-sm"><i class="fas fa-trash-alt"></i> Eliminar</button></div></div>');
